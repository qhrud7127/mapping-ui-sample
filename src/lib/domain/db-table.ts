...
import {decodeBase64ToUtf8, deepCopy, generateId,} from '../utils';
import {TableInfo} from "../../data/metadata-types/table-info.ts";
import {materializedViewColor, randomColor, viewColor} from "../colors.ts";
import {DatabaseMetadata} from "../../data/metadata-types/database-metadata.ts";
...
export const decodeViewDefinition = (
  ...
  return decodeBase64ToUtf8(viewDefinition);
};
  ...
  return tableInfos.map((tableInfo: TableInfo) => {
    ...
    const isMaterializedView = views.some(
      (view) =>
        ...
        decodeViewDefinition(view.view_definition)
          .toLowerCase()
